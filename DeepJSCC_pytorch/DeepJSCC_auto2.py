...
for i, SNR in enumerate(SNR_list):
    ...
    for j, x in enumerate(x_list):
        ...
        model.add(Layers2.Normalization(k, P))
        # model.add(Layers2.Modulation())
        # model.add(Layers2.Demodulation())

        # add channel noise (AWGN)
        if slow_rayleigh_fading == True:
            model.add(Layers2.Slow_Rayleigh_Fading_Channel())
        model.add(Layers2.AWGN_Channel(N))
