...
for i, SNR in enumerate(SNR_list):
    ...
    for j, x in enumerate(x_list):
        ...
        dis = models.Sequential(name="Discriminator")

        dis.add(layers.Conv2D(32, (5, 5), stride=1, padding="same"))
        dis.add(layers.PReLU())
        dis.add(layers.Conv2D(128, (5, 5), stride=2, padding="same"))
        dis.add(layers.PReLU())
        dis.add(layers.Conv2D(256, (5, 5), stride=2, padding="same"))
        dis.add(layers.PReLU())
        dis.add(layers.Conv2D(256, (5, 5), stride=2, padding="same"))
        dis.add(layers.PReLU())
